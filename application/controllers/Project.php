...
class Project extends CI_Controller
{
	...
	public function select_subTask_superTask()
	{
		...
		$task_id = $_GET['taskID'];
		//echo $task_id;
		$subtask_id = $_GET['subTaskID'];

		if($task_id != ''){ 
			$sub_tasks_sql = "SELECT sub_tasks.* FROM sub_tasks WHERE sub_tasks.task_id = ".$task_id; 
			$sub_tasks_query = $this->db->query($sub_tasks_sql); 
			$sub_tasks_result = $sub_tasks_query->result_array();
			// Generate HTML of state options list 
			if($sub_tasks_query->num_rows() > 0){ 
				echo '<option value="">Select Sub-task</option>'; 
				foreach($sub_tasks_result as $task_resultRow){ 
					echo '<option value="'.$task_resultRow['id'].'">'.$task_resultRow['name'].'</option>'; 
				} 
			}else{ 
				echo '<option value="">Sub-Task not available</option>'; 
			} 
		}elseif($subtask_id != ''){ 
			// Fetch city data based on the specific state 
			$sub_task_sql = "SELECT * FROM super_sub_task WHERE super_sub_task.subtask_id = ".$subtask_id; 
			$sub_task_query = $this->db->query($sub_task_sql); 
			//$subtask_num_rows = $subtask_result->num_rows();
			$subtask_result = $sub_task_query->result_array();
			
			
			if($sub_task_query->num_rows() > 0){ 
				echo '<option value="">Select Super Task</option>'; 
				foreach($subtask_result as $supertask_resultRow){ 
					echo '<option value="'.$supertask_resultRow['id'].'">'.$supertask_resultRow['name'].'</option>'; 
				} 
			}else{ 
				echo '<option value="">Super Task not available</option>'; 
			} 
		} 
	}
